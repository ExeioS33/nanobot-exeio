...
export function channelSetup(
  ...
  const presentation: ChannelSetupPresentation = {
    ...
    summary:
      ...
      ?? "Enable turns on this channel in Exeio, but this integration still needs platform-specific setup before it can receive messages.",
    ...
    steps: setupMessages?.steps ?? [
      ...
      "Restart Exeio, then send a small test message from that platform.",
    ],
...
export function channelDescription(feature: NanobotFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
    `Use Exeio from ${channelDisplayName(feature)}.`;
